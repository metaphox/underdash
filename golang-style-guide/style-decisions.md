...
Avoid selecting package names that are likely to be [shadowed](best-practices#shadowing) by commonly used
...
Constant names must use [MixedCaps](guide#mixed-caps) like all other names in Go. ([Exported](https://tour.golang.org/basics/3)
...
judgement based on context, [clarity](guide#clarity), and [concision](guide#concision).
...
* Omit words that are clear from the [surrounding context](#repetitive-in-context). For example, in
...
* For a [method receiver variable](#receiver-names), a one-letter or two-letter name is
  preferred.
...
The best practices document’s section on [documentation conventions](best-practices#documentation-conventions) discusses
this further.

**Best Practice:** Use [doc preview](best-practices#documentation-preview) during development and code review to see
...
should be [full sentences](#comment-sentences) that begin with the name of the object being
...
[Documentation comments](#doc-comments) should always be complete sentences, and as such should
...
test file, not the production source file. See this example ([Godoc](https://pkg.go.dev/time#example-Duration), [source](https://cs.opensource.google/go/go/%2B/HEAD%3Asrc/time/example_test.go)).
...
4. Import for [side-effects](https://go.dev/doc/effective_go#blank_import)
...
[best practices](/styleguide/go/best-practices.html#error-handling).
...
Structs within table-driven tests often benefit from [explicit field names](#literal-field-names),
...
Also see [when panic is acceptable](/styleguide/go/best-practices.html#when-to-panic).
...
[synchronous functions](#synchronous-functions). If the concurrency is still not obvious, it is
...
* [Documentation Conventions: Contexts](/styleguide/go/best-practices.html#documentation-conventions-contexts)
...
* Do not define back doors or export [test double](https://abseil.io/resources/swe-book/html/ch13.html) implementations of an
  interface solely for testing. Prefer testing via the [public API](https://abseil.io/resources/swe-book/html/ch12.html#test_via_public_apis) of the
...
documented. It’s highly encouraged to include motivating runnable [examples](#examples).
...
[*type alias*](http://golang.org/ref/spec#Type_declarations), `type T1 = T2`, to refer to an existing type without defining a
...
Go 1.18 introduces an `any` type as an [alias](https://go.googlesource.com/proposal/%2B/master/design/18130-type-alias.md) to `interface{}`. Because it is
...
should follow the standard Go name style ([mixed caps](guide#mixed-caps)). Specifically, the flag
...
There is additional discussion around best practices for creating [complex CLIs](best-practices#complex-clis)
with subcommands.
...
* In other entrypoint functions (see below for examples of such functions),
  use [`context.Background()`](https://pkg.go.dev/context/#Background).
...
Complex assertion functions often do not provide [useful failure messages](#useful-test-failures) and
...
[section on printing diffs](#print-diffs). Whichever diff order you use in your failure
...
tests and new code, but other tools may be used. See [types of equality](#types-of-equality) for
...
To [identify the inputs](#identify-the-input) of the function, include them in the test’s failure
...
* When testing whether the actual output of a function is equal to the
  expected output. For example, the many [tests of `fmt.Sprintf`](https://cs.opensource.google/go/go/%2B/master%3Asrc/fmt/fmt_test.go) or the
  minimal snippet below.
* When testing whether the outputs of a function always conform to the same
  set of invariants. For example, [tests for `net.Dial`](https://cs.opensource.google/go/go/%2B/master%3Asrc/net/dial_test.go;l=318;drc=5b606a9d2b7649532fe25794fa6b99bd24e7697c).
...
always [identify the input](#identify-the-input). The table test row names must follow the
[subtest naming](#subtest-names) guidance.
...
[assert libraries](#assert) still applies, and [`t.Helper`](https://pkg.go.dev/testing#T.Helper) should not be used
...
[examples](#examples) declared in the test will not have the package names that a user will
need in their code.
...
This site is open source. [Improve this page](https://github.com/google/styleguide/edit/gh-pages/go/decisions.md).